...
export default function PortfolioView({ initialData }: PortfolioViewProps) {
  ...
  return (
    <main className="min-h-screen bg-[#070b14] text-slate-100 selection:bg-blue-900/50 selection:text-blue-100 antialiased font-sans relative">
      ...
      {/* HEADER / NAVIGATION */}
      <header className="sticky top-0 z-50 backdrop-blur-md bg-[#070b14]/85 border-b border-slate-800/60">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <a href="#" className="font-semibold text-base tracking-tight hover:text-blue-300 transition shrink-0">
            {profile.name}
          </a>
          ...
          <nav className="flex items-center space-x-6 text-sm text-slate-400">
            <div className="hidden md:flex items-center space-x-6">
              ...
              <a href="#architecture" className="hover:text-slate-100 transition">{UI_TEXT.nav.architecture}</a>
            ...
            <div className="flex items-center bg-slate-900/90 shadow-inner rounded-md p-0.5 text-xs ring-1 ring-slate-800">
              <button
                onClick={() => setLang('vie')}
                className={`flex items-center gap-1.5 px-2 py-1 rounded transition-all duration-150 text-xs cursor-pointer ${
                  lang === 'vie'
                    ? 'bg-slate-800 text-white font-medium shadow-sm ring-1 ring-blue-500/30'
                    : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/40'
                }`}
                aria-label="Tiếng Việt"
              >
                <VietnamFlag className="w-5 h-3.5 shrink-0 shadow-sm" />
                <span className="font-mono text-xs font-semibold leading-none">VI</span>
              </button>
              <button
                onClick={() => setLang('eng')}
                className={`flex items-center gap-1.5 px-2 py-1 rounded transition-all duration-150 text-xs cursor-pointer ${
                ...
                aria-label="English"
              >
                <UkFlag className="w-5 h-3.5 shrink-0 shadow-sm" />
                <span className="font-mono text-xs font-semibold leading-none">EN</span>
              </button>
            </div>
          </nav>
        </div>
      ...
      <section id="about" className="relative min-h-screen flex items-center overflow-hidden border-b border-slate-800/60">
      ...
      <section id="experience" className="max-w-6xl mx-auto px-6 py-20 border-t border-slate-800/60">
        <MotionReveal>
          <div className="flex flex-col md:flex-row md:items-baseline justify-between mb-16 gap-3 pb-6 border-b border-slate-800/60">
            <div>
          ...
          {experiences.map((exp, index) => {
            ...
            return (
              <MotionReveal key={exp.id} delay={index * 0.1}>
                <div className="group p-6 md:p-8 rounded-xl bg-[#0b1222]/75 hover:bg-[#0f1930]/90 shadow-xl shadow-[#020617]/80 hover:shadow-2xl hover:shadow-blue-950/40 hover:-translate-y-1 transition-all duration-300 ring-1 ring-blue-500/10 hover:ring-blue-400/25 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative overflow-hidden backdrop-blur-md">
                  ...
                  <div className="lg:col-span-4 space-y-3">
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-1 font-mono text-xs">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md ring-1 shadow-sm ${
                        isCurrent
                          ? 'bg-emerald-950/60 text-emerald-300 ring-emerald-700/60 shadow-emerald-950/50'
                          : 'bg-[#060b17] text-slate-400 ring-slate-800'
                      }`}>
                        {isCurrent && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />}
                        {exp.startDate} — {exp.endDate}
                      </span>
                      <span className="px-2.5 py-1 rounded-md bg-[#060b17] text-slate-400 ring-1 ring-slate-800">
                        {exp.location}
                      </span>
                    </div>
                  </div>
      ...
      <section id="architecture" className="max-w-6xl mx-auto px-6 py-20 border-t border-slate-800/60">
        <MotionReveal>
          <div className="flex flex-col md:flex-row md:items-baseline justify-between mb-12 gap-3 pb-6 border-b border-slate-800/60">
            <div>
              <span className="text-xs font-mono text-blue-400 uppercase tracking-wider block mb-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {projects.map((proj, pIdx) => (
            <MotionReveal key={proj.id} delay={pIdx * 0.1}>
              <div className="group rounded-xl bg-[#0b1222]/75 hover:bg-[#0f1930]/90 p-6 md:p-8 flex flex-col justify-between h-full shadow-xl shadow-[#020617]/80 hover:shadow-2xl hover:shadow-blue-950/40 hover:-translate-y-1.5 transition-all duration-300 ring-1 ring-blue-500/10 hover:ring-blue-400/25 relative overflow-hidden backdrop-blur-md">
                ...
                <div className="pt-6 mt-6 border-t border-slate-800/60 space-y-4">
          ...
          {matrix.map((col, idx) => {
            ...
            return (
              <MotionReveal key={col.layer} delay={idx * 0.08}>
                <div className="group p-6 rounded-xl bg-[#0b1222]/75 hover:bg-[#0f1930]/90 shadow-xl shadow-[#020617]/80 hover:shadow-2xl hover:shadow-blue-950/40 hover:-translate-y-1.5 transition-all duration-300 ring-1 ring-blue-500/10 hover:ring-blue-400/25 space-y-4 relative overflow-hidden backdrop-blur-md">
                  ...
                  <div className="space-y-1.5 pt-2">
                    {col.skills.map((s) => (
                      <div
                        key={s}
                        className="text-xs font-mono text-slate-300 py-1 border-b border-slate-800/50 last:border-0 group-hover:text-blue-200 transition-colors"
                      >
                        {s}
                      </div>
                    ))}
            ...
            {beyondWork.items.map((item, bIdx) => {
              return (
                <MotionReveal key={item.id} delay={bIdx * 0.1}>
                  <div className="group rounded-xl bg-[#0b1222]/75 hover:bg-[#0f1930]/90 p-6 md:p-8 flex flex-col justify-between h-full space-y-4 shadow-xl shadow-[#020617]/80 hover:shadow-2xl hover:shadow-blue-950/40 hover:-translate-y-1.5 transition-all duration-300 ring-1 ring-blue-500/10 hover:ring-blue-400/25 relative overflow-hidden backdrop-blur-md">
                    <div className="space-y-3">
      ...
      {/* FOOTER */}
      <footer className="border-t border-slate-800/60 py-8 bg-[#070b14] text-center text-xs text-slate-500 font-mono">
